...
type Variant = "waves" | "kinetic" | "lattice" | "stars" | "neural";
...
export default function SectionBackground({ variant, section }: SectionBackgroundProps) {
  ...
  const starsRef = useRef<{ x: number; y: number; pulse: number }[] | null>(null);
  ...
  const kineticRipplesRef = useRef<{ x: number; y: number; born: number }[]>([]);
  const waveParticlesRef = useRef<
    { x: number; speed: number; size: number; alpha: number; waveIndex: number }[] | null
  >(null);
  const tRef = useRef(0);
  ...
  const paint = useCallback(() => {
    ...
    if (variant === "lattice") {
      ...
      const points = latticeRef.current;
    ...
    if (variant === "stars") {
      ...
      if (starsRef.current === null) {
        ...
        starsRef.current = nodes;
      }
      if (starsMeteorsRef.current === null) {
        const spawn = (): Meteor => ({
          x: Math.random() * W,
          y: Math.random() * H * 0.5,
        ...
        starsMeteorsRef.current = Array.from({ length: 6 }, () => spawn());
      }
      const nodes = starsRef.current;
      const meteors = starsMeteorsRef.current;
      ...
      if (!!section && activeMeteors < 3 && Math.random() < 0.008) {
    ...
    if (variant === "neural") {
      ...
      if (neuralNodesRef.current === null) {
        neuralNodesRef.current = Array.from({ length: NEURAL_COUNT }, () => ({
          x: Math.random() * W,
          y: Math.random() * H,
      ...
      if (neuralSignalsRef.current === null) {
        ...
        neuralSignalsRef.current = Array.from({ length: 4 }, () => spawn());
      }
      const nodes = neuralNodesRef.current;
      const signals = neuralSignalsRef.current;
    ...
    if (variant === "waves") {
      ...
      waves.forEach((wave, i) => {
        ...
        for (let x = 0; x <= W; x += 2) {
          const mouseWarp = hasMouse
            ...
            : 0;
          const y = H * wave.yOffset + Math.sin(x * wave.freq + tRef.current * wave.speed) * wave.amp + mouseWarp + mouseYInfluence * (i * 0.3);
          ctx.lineTo(x, y);
        }
        ctx.lineTo(W, H);
        ctx.lineTo(0, H);
      ...
      if (waveParticlesRef.current === null) {
        waveParticlesRef.current = Array.from({ length: 25 }, () => ({
          x: Math.random(),
      ...
      const waveParticles = waveParticlesRef.current;
      ...
      waveParticles.forEach((p) => {
        ...
        if (!wave) return;
        const px = p.x * W;
        const py = H * wave.yOffset + Math.sin(px * wave.freq + tRef.current * wave.speed) * wave.amp;
        ctx.beginPath();
      ...
      const scanY = ((Math.sin(tRef.current * 0.3) + 1) / 2) * H;
      ctx.beginPath();
      ctx.moveTo(0, scanY);
      ctx.lineTo(W, scanY);
