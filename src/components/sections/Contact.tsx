...
const socialLinks = [
  {
    label: "GitHub",
    url: links.github,
    color: "hover:border-zinc-400/40 hover:text-zinc-200",
    icon: (
  ...
  },
  {
    ...
    color: "hover:border-[#00d0af]/40 hover:text-[#00d0af]",
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
  ...
  },
  {
    label: "ORCID",
    url: links.orcid,
    color: "hover:border-[#a6ce39]/40 hover:text-[#a6ce39]",
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.5 }}
            className="glass-blue rounded-2xl p-8 relative overflow-hidden"
          >
            <div className="absolute top-0 right-0 w-40 h-40 bg-[#3b82f6]/6 rounded-full blur-2xl pointer-events-none" />

            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl glass flex items-center justify-center text-[#3b82f6]">
              ...
              <div>
                <p className="text-sm font-semibold text-zinc-100">{t.email_label}</p>
                <p className="text-xs text-zinc-500">{t.email_desc}</p>
              </div>
            </div>

            <p className="text-sm text-[#93c5fd] mb-6 font-mono">{links.email}</p>

            <a
              href={`mailto:${links.email}`}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[#3b82f6] hover:bg-[#2563eb] text-white text-sm font-medium transition-colors duration-200"
            >
          ...
          >
            <p className="text-xs text-zinc-600 uppercase tracking-widest mb-1">{t.or}</p>
            ...
            {socialLinks.map((social, i) => (
              <motion.a
                ...
                className={`glass rounded-xl px-5 py-4 flex items-center gap-4 border border-white/[0.06] text-zinc-400 transition-all duration-200 ${social.color}`}
              >
                <span className="flex-shrink-0">{social.icon}</span>
                <span className="text-sm font-medium">{social.label}</span>
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="ml-auto opacity-40">
